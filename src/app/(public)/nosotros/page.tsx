...
export default function AboutPage() {
    ...
    return (
        <main className="min-h-screen bg-[#f8f9fa] dark:bg-[#0A0F0D] overflow-hidden">
            ...
            <section className="py-24 px-6 max-w-7xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div className="space-y-8 animate-in animate-delay-1">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-sky-50 dark:bg-[#111A15] border border-sky-100 dark:border-[#2A3F33] rounded-full">
                        ...
                        <h2 className="text-4xl md:text-5xl font-black text-slate-800 dark:text-[#E8EDE9] leading-[1.1]">
                        ...
                        <div className="space-y-6 text-slate-600 dark:text-[#7A8F7F] leading-relaxed text-lg">
                            {aboutData.doctorSection.paragraphs.map((p, i) => (
                                <p key={i} className={i === 0 ? "font-semibold text-slate-900 dark:text-[#E8EDE9] border-l-4 border-sky-500 pl-6 py-2 bg-sky-50/30 dark:bg-[#111A15]/30 rounded-r-2xl" : ""}>
                                    {p}
                                </p>
                            ))}
                        </div>
                        ...
                        <div className="pt-4 flex items-center gap-6">
                            <div className="flex -space-x-3">
                                {[1, 2, 3, 4].map(i => (
                                    <div key={i} className="w-12 h-12 rounded-full border-4 border-white dark:border-[#0A0F0D] bg-slate-200 dark:bg-[#182420] overflow-hidden shadow-lg transform hover:-translate-y-1 transition-transform">
                                        <Image src={`https://i.pravatar.cc/150?u=${i}`} alt="user" width={48} height={48} className="w-full h-full object-cover" />
                                    </div>
                                ))}
                            </div>
                            <p className="text-sm font-bold text-slate-500 dark:text-[#7A8F7F]">
                    ...
                    <div className="relative group animate-in animate-delay-2">
                        ...
                        <div className="relative aspect-[4/5] md:aspect-square lg:h-[600px] rounded-[2.5rem] overflow-hidden shadow-2xl border-8 border-white dark:border-[#111A15]">
                            <Image
                            ...
                            />
                            <div className="absolute bottom-6 left-6 right-6 p-6 bg-white/90 dark:bg-[#111A15]/90 backdrop-blur-md rounded-2xl shadow-xl border border-white/50 dark:border-[#2A3F33]">
                                <p className="text-sky-600 dark:text-[#6BAF7B] font-black uppercase text-xs tracking-widest mb-1">Especialistas en BioSalud</p>
                                <h4 className="font-bold text-slate-800 dark:text-[#E8EDE9] text-lg">Compromiso Lyrium</h4>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="py-32 bg-white dark:bg-[#111A15] relative">
                <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-[#f8f9fa] dark:from-[#0A0F0D] to-transparent" />

                <div className="max-w-7xl mx-auto px-6 relative">
                    <div className="text-center mb-20 space-y-4">
                        <div className="inline-block p-3 bg-sky-100 dark:bg-[#182420] rounded-2xl text-sky-600 dark:text-[#6BAF7B] mb-4 animate-bounce">
                            <Icon name="Star" className="w-6 h-6 fill-sky-600 dark:fill-[#6BAF7B]" />
                        </div>
                        <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-[#E8EDE9] tracking-tighter">
                            ...
                            {aboutData.timeline.items.map((item, idx) => (
                                <div key={item.title} className={`flex flex-col md:flex-row items-center justify-center w-full md:mb-12 ${idx % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
                                    <div className="w-full md:w-5/12 hidden md:block group">
                                        <div className={`p-8 bg-sky-50/50 dark:bg-[#182420]/50 border-2 border-transparent hover:border-sky-200 dark:hover:border-[#2A3F33] hover:bg-white dark:hover:bg-[#111A15] rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-500 relative ${idx % 2 === 0 ? 'text-right' : 'text-left'}`}>
                                            <div className={`absolute top-1/2 -translate-y-1/2 ${idx % 2 === 0 ? '-right-4' : '-left-4'} w-8 h-8 bg-white dark:bg-[#111A15] border-2 border-sky-200 dark:border-[#2A3F33] rounded-lg rotate-45 z-0`} />
                                            <h3 className="text-2xl font-black text-slate-800 dark:text-[#E8EDE9] mb-3 uppercase tracking-tight relative z-10">{item.title}</h3>
                                            <p className="text-slate-500 dark:text-[#7A8F7F] font-medium leading-relaxed relative z-10">{item.description}</p>
                                    ...
                                    <div className="relative z-20 flex items-center justify-center w-full md:w-2/12 mb-8 md:mb-0">
                                        <div className="w-16 h-16 bg-white dark:bg-[#111A15] border-4 border-sky-500 dark:border-[#4A7C59] rounded-full shadow-[0_0_20px_rgba(14,165,233,0.3)] dark:shadow-[0_0_20px_rgba(74,124,89,0.3)] flex items-center justify-center p-4 group hover:scale-110 transition-all duration-500 cursor-help">
                                            {getTimelineIcon(idx)}
                                        </div>
                                    </div>

                                    <div className="w-full md:w-5/12 block group">
                                        <div className="md:hidden block p-8 bg-white dark:bg-[#111A15] border-2 border-sky-100 dark:border-[#2A3F33] rounded-[2rem] shadow-lg text-center relative overflow-hidden">
                                            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-1 bg-sky-500" />
                                            <h3 className="text-xl font-black text-slate-800 dark:text-[#E8EDE9] mb-3 uppercase tracking-tighter">{item.title}</h3>
                                            <p className="text-slate-500 dark:text-[#7A8F7F] font-medium text-sm leading-relaxed">{item.description}</p>
            ...
            <section className="py-32 bg-white dark:bg-[#111A15] relative overflow-hidden">
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-64 h-64 bg-sky-50 dark:bg-[#182420] rounded-full blur-[100px] -z-10" />
                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-64 h-64 bg-sky-50 dark:bg-[#182420] rounded-full blur-[100px] -z-10" />
                        ...
                        {aboutData.premiumIcons.map((icon, idx) => (
                            <div key={icon.title} className="flex flex-col items-center text-center group animate-in" style={{ animationDelay: `${idx * 150}ms` }}>
                                <div className="relative w-32 h-32 md:w-48 md:h-48 mb-8">
                                    <div className="absolute -inset-4 border-2 border-dashed border-sky-100 dark:border-[#2A3F33] rounded-full group-hover:rotate-180 transition-transform duration-[4000ms]" />
                                    <div className="absolute -inset-2 border border-sky-200/50 dark:border-[#2A3F33]/50 rounded-full group-hover:-rotate-180 transition-transform duration-[6000ms]" />

                                    <div className="relative w-full h-full bg-white dark:bg-[#111A15] rounded-full shadow-2xl flex items-center justify-center p-6 border-2 border-sky-50 dark:border-[#2A3F33] group-hover:shadow-[0_20px_50px_rgba(14,165,233,0.15)] dark:group-hover:shadow-[0_20px_50px_rgba(74,124,89,0.15)] transition-all duration-500 transform group-hover:-translate-y-3">
                                        <Image
                                            src={`/${icon.image}`}
                                            alt={icon.title}
                                            width={160}
                                ...
                                <h3 className="text-xl md:text-2xl font-black text-slate-800 dark:text-[#E8EDE9] mb-4 tracking-tight group-hover:text-sky-500 dark:group-hover:text-[#6BAF7B] transition-colors">
                                    {icon.title}
                                </h3>
                                <p className="text-sm md:text-base text-slate-500 dark:text-[#7A8F7F] font-medium leading-relaxed max-w-[220px]">
                                    {icon.description}
                                </p>
                            </div>
                        ))}
