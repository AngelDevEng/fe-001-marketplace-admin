...
export default function CheckoutStepBar() {
    ...
    return (
        <div id="checkout-step-bar" className="bg-white relative z-30">
                            ...
                            <div key={step.id} className={`flex items-center ${idx < STEPS.length - 1 ? 'flex-1' : ''}`}>
                                ...
                                >
                                    ...
                                    <div className="relative group transition-transform duration-500 hover:scale-110">
                                        ...
                                        />
                                        <div className="relative w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-md border border-gray-100/50">
                                            <Icon
                                                className={`w-5 h-5 transition-colors duration-500 ${isActive ? 'text-sky-600' : isCompleted ? 'text-emerald-500' : 'text-gray-400'
                                                    }`}
                                            />
                                        </div>
                                    </div>
                                ...
                                {idx < STEPS.length - 1 && (
                                    <div className="flex-1 h-3 flex items-start px-2 relative z-0 -mt-10"> {/* Negative margin to align with circle vertical center */}
                                        <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden transition-all duration-500">
                                            <div
