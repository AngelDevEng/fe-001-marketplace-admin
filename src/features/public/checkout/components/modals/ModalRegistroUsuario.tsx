...
export default function ModalRegistroUsuario({ isOpen, email, onClose }: Props) {
    ...
    return (
        <>
            {/* Backdrop */}
            <div className="fixed inset-0 bg-white/80 backdrop-blur-sm z-[1400] animate-fade-in" />

            {/* Modal */}
            <div className="fixed inset-0 z-[1600] flex items-center justify-center p-4 animate-fade-in">
                <div className="bg-white w-full max-w-md shadow-2xl relative flex flex-col rounded-[2.5rem] overflow-hidden animate-modal-pop max-h-[90vh]">
                    ...
                    {/* Body */}
                    <div className="bg-white p-5 space-y-4 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:#bae6fd_transparent]">
                        <div className="space-y-3">
                            <div className="text-center space-y-1.5">
                                <span className="text-[8px] font-black text-sky-500 uppercase tracking-[0.3em]">Protocolo de Activación</span>
                                <h2 className="text-xl font-black text-gray-900 tracking-tight">Reclama tu panel personal.</h2>
                                <p className="text-[11px] text-gray-400 font-medium leading-relaxed">Establece tu llave de acceso privada para gestionar tus pedidos.</p>
                            </div>
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    { icon: '🚚', label: 'Tracking', bg: 'bg-sky-50 border-sky-100 text-sky-500' },
                                    { icon: '🧾', label: 'Facturas', bg: 'bg-emerald-50 border-emerald-100 text-emerald-500' },
                                    { icon: '🌐', label: 'Global', bg: 'bg-purple-50 border-purple-100 text-purple-500' },
                                ].map(({ icon, label, bg }) => (
                                    <div key={label} className={`p-2.5 rounded-xl border flex flex-col items-center text-center gap-1 group ${bg}`}>
                                        <span className="text-lg group-hover:scale-110 transition-transform">{icon}</span>
                                        <span className="text-[7px] font-bold text-gray-800 uppercase tracking-wider">{label}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-3">
                            <div className="space-y-1.5">
                                <label className="text-[8px] font-bold text-gray-400 uppercase tracking-widest pl-1">E-mail Detectado</label>
                                <div className="px-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-gray-500 font-bold text-sm flex items-center gap-3">
                            ...
                            <div className="space-y-1.5 group">
                                <label className="text-[8px] font-bold text-gray-400 uppercase tracking-widest pl-1">Crea tu llave de acceso</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-10 py-3.5 bg-white border border-gray-200 rounded-xl text-gray-900 font-bold text-sm focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"
                            ...
                            <div className="pt-1.5 space-y-2.5">
                                <button
                                    ...
                                    className={`w-full py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all shadow-lg flex items-center justify-center gap-2 ${status === 'success'
                                        ...
                                        : 'bg-gray-900 text-white hover:bg-sky-500 hover:-translate-y-0.5 active:translate-y-0'
                                        } disabled:opacity-70`}
                                >
                                ...
                                <div className="flex flex-col items-center gap-1.5 pt-1">
                                    ...
                                    <button type="button" onClick={onClose} className="text-[8px] font-bold text-gray-300 uppercase tracking-widest hover:text-rose-500 transition-all">
