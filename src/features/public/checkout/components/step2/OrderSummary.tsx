...
import type { DeliveryMethod, PaymentMethod } from '../../hooks/useCheckoutStore';
...
const inputCls = "w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100 transition-all";
...
export default function OrderSummary() {
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm sticky top-[100px]">
            ...
            <div className="p-5 space-y-4">
                ...
                {/* Save payment toggle */}
                <div className="p-4 bg-purple-50/60 rounded-2xl border border-purple-100/40">
                    <label className="flex items-center gap-3 cursor-pointer group select-none">
                        <div className="relative inline-flex items-center">
                            ...
                            <div className="w-10 h-6 bg-gray-200 rounded-full peer peer-checked:bg-purple-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full transition-all" />
                        </div>
                        <span className="text-xs font-bold text-purple-900/70 group-hover:text-purple-600 transition-colors">
                ...
                <div className="flex gap-2">
                    <input type="text" placeholder="Código promocional" value={orderData.promoCode} onChange={(e) => setOrderData({ promoCode: e.target.value })} className="flex-1 px-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-sky-400 focus:outline-none transition-all" />
                    <button type="button" className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-bold text-xs transition-all">
                        Aplicar
                    </button>
                </div>
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between text-gray-500">
                        <span>Descuento</span>
                        <span className="font-semibold text-emerald-600">-S/ {orderData.discount.toFixed(2)}</span>
                    </div>
                </div>
