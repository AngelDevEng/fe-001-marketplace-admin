...
import type { DocType } from '../../hooks/useCheckoutStore';
    ...
    return (
        <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-tighter ${required ? 'text-red-500 bg-red-50' : 'text-gray-400 bg-gray-100'}`}>
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Field label="Documento de Identidad" required={isCE || isPAS}>
                        <div className="flex gap-2">
                            <select
                                ...
                                className="w-28 px-3 py-3 border-2 border-gray-200 rounded-xl text-sm bg-gray-50 focus:bg-white focus:border-sky-400 focus:outline-none transition-all font-bold text-gray-700"
                            >
                ...
                {/* CE extra fields */}
                {isCE && (
                    <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-sky-50/50 rounded-2xl border-2 border-dashed border-sky-100">
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-sky-600 uppercase tracking-wide">Nacionalidad</label>
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-sky-600 uppercase tracking-wide">Vencimiento CE</label>
                            <input type="date" value={data.ceVencimiento} onChange={(e) => setPersonalData({ ceVencimiento: e.target.value })} className={inputCls} />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-sky-600 uppercase tracking-wide">Calidad Migratoria</label>
                ...
                <Field label="Celular Principal" required>
                    <div className="flex gap-2">
                        {isPAS && (
                            <select value={data.celularPrefix} onChange={(e) => setPersonalData({ celularPrefix: e.target.value })} className="w-24 px-3 py-3 border-2 border-gray-200 rounded-xl text-sm bg-gray-50 focus:border-sky-400 focus:outline-none transition-all text-center font-bold text-sky-600">
