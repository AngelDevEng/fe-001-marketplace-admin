...
export default function CheckoutPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 antialiased">
            <div id="lyrium-stack-wrapper" className="min-h-screen bg-white">

                {/* Sticky top wrapper */}
                <div id="checkout-top-wrapper" className="sticky top-0 z-[10000] bg-white border-b border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
                    <CheckoutStepBar />
                </div>
                ...
                {isProcessing && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[1700] flex items-center justify-center px-4">
                        <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center space-y-6 animate-modal-pop">
                            ...
                            <div className="space-y-2">
                                <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight">Procesando Pedido</h3>
                                <p className="text-[10px] text-sky-400 font-bold uppercase tracking-[0.2em] animate-pulse">Encriptando transacción...</p>
                            </div>
                            <div className="flex items-center justify-center gap-2">
                                {[0.1, 0.2, 0.3].map((d) => (
                                    <span key={d} className="w-1.5 h-1.5 rounded-full bg-sky-500 animate-bounce" style={{ animationDelay: `${d}s` }} />
                                ))}
                            </div>
                        </div>
                    </div>
