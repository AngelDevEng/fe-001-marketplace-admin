                                    ...
                                    <tr key={v.id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-gray-800 font-mono tracking-tight">{v.series}-{v.number}</p>
                                                    <p className="text-[10px] font-black text-gray-400 uppercase bg-gray-100 w-fit px-1.5 rounded-md">{v.type}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-bold text-gray-700 truncate max-w-[200px]">{v.customer_name}</p>
                                            <p className="text-[10px] text-gray-400 font-black font-mono">{v.customer_ruc}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-xs font-black text-sky-600 bg-sky-50 px-2 py-1 rounded-lg border border-sky-100">{v.order_id}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-gray-800">S/ {v.amount.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button
                                                ...
                                                className="w-10 h-10 bg-white border border-gray-100 text-gray-400 hover:text-emerald-600 hover:border-emerald-200 rounded-xl transition-all shadow-sm active:scale-90 flex items-center justify-center m-auto mr-0"
                                            >
