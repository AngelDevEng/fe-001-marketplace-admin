...
export default function PlanModal({
    ...
    const [localCustomMonths, setLocalCustomMonths] = useState(customMonths);

    useEffect(() => {
        setLocalCustomMonths(customMonths);
    }, [customMonths]);
    ...
    const handleCustomMonthsChange = (delta: number) => {
        const newVal = localCustomMonths + delta;
        if (newVal >= 4 && newVal <= 48) {
            setLocalCustomMonths(newVal);
        ...
        >
            {step === 1 && (
                <div className="space-y-6">
                    <div>
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Selecciona la duración</h4>
                    ...
                    <div className="p-4 bg-gray-50 rounded-xl space-y-3">
                        ...
                        {summary.discount > 0 && (
                            <>
                                <div className="flex justify-between text-gray-400">
                                    ...
                                    <span className="line-through">{formatPrice(summary.originalPrice, plan.currency)}</span>
                                </div>
                                <div className="flex justify-between text-green-600">
                                    <span>Descuento</span>
                                    <span className="font-semibold">-{formatPrice(summary.discount, plan.currency)}</span>
                                </div>
                            </>
                        )}
                        ...
                        {!isTrial && summary.perMonth > 0 && (
                            <div className="flex justify-between text-sm text-gray-500">
                                <span>Equivale a</span>
                                <span>{formatPrice(summary.perMonth, plan.currency)}/mes</span>
                            </div>
                        )}
                    </div>
