...
import { hexToRgba, lightenColor, formatPrice, isMobile } from '@/features/seller/plans-lyrium/lib/helpers';
...
export default function Showcase({ showcasePlan, plansData, planOrder, currentPlan, claimedPlans, hasPendingRequest, onOpenPayment, onClaimFree, onOpenDowngrade, onFeatureClick }: Props) {
  ...
  const maxShow      = showcaseExpanded ? data.features.length : configLimit;
  ...
  if (data.bgImage) {
    ...
    bgStyle = { background:`linear-gradient(135deg, ${lightenColor(data.cssColor, 0.85)} 0%, ${lightenColor(data.cssColor, 0.7)} 40%, ${lightenColor(data.cssColor, 0.4)} 100%)` };
  }
  ...
  const badgeBg     = hexToRgba(data.cssColor, 0.12);
  const badgeColor  = data.accentColor ?? data.cssColor;
  const badgeBorder = hexToRgba(data.cssColor, 0.37);
  ...
  return (
    <div className="plan-showcase" id="planShowcase">
      ...
      <div className="showcase-content" style={{ animation:'showcasePlanChange 1.8s cubic-bezier(0.16,1,0.3,1)' }}>
        ...
        <h2 className="showcase-plan-name animate-showcase-el" style={{ color:lightenColor(data.cssColor, -0.1), animationDelay:'0.15s' }}>{data.name}</h2>
        ...
        <div className="showcase-features animate-showcase-el" style={{ animationDelay:'0.3s' }}>
          {data.features.slice(0, maxShow).map((f, i) => (
            <div key={`${showcasePlan}-sf-${i}`} className={`showcase-feature ${f.active ? 'sf-active' : 'sf-inactive'} feature-clickable`} onClick={() => onFeatureClick(showcasePlan)}>
              <span className={`sf-icon ${f.active ? 'active' : 'inactive'}`}>{f.active ? '✓' : '✕'}</span>
              <span>{f.text}</span>
            </div>
          ))}
        </div>
        {data.features.length > configLimit && (
          <button className="showcase-toggle-btn" onClick={() => setShowcaseExpanded(!showcaseExpanded)}>
            {showcaseExpanded
              ...
              : <><span>Ver {data.features.length - configLimit} beneficios más</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg></>
            }
          </button>
        )}
