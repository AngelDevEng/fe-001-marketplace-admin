...
import { benefitDetailsFallback } from '@/features/seller/plans-lyrium/lib/benefitDetails';
import type { PlansMap, DetailedBenefit } from '@/features/seller/plans-lyrium/types';
...
export function BenefitFullModal({ open, planKey, plansData, onClose }: BenefitFullProps) {
  const data = plansData[planKey];
  // Igual que el original: si detailedBenefits de BD está vacío, usar el fallback hardcodeado
  const details: (DetailedBenefit | { emoji?: string; icon?: string; title: string; description: string; color?: string })[] =
    (data?.detailedBenefits?.length ?? 0) > 0
      ? data!.detailedBenefits
      : (benefitDetailsFallback[planKey] ?? []);
      ...
      <div className="benefit-detail-list">
        {details.length === 0
          ...
          : details.map((d, i) => {
              ...
              return (
                <div key={`bd-${i}-${d.title.slice(0,6)}`} className="benefit-detail-item animate-benefit-item" style={{ animationDelay:`${i * 0.08}s` }}>
