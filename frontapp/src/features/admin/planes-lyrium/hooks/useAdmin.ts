...
import { apiGet, apiPost, silentPost } from '@/features/seller/plans-lyrium/lib/api';
import { defaultPlansData, buildPlanOrder } from '@/features/seller/plans-lyrium/lib/plans';
import { formatAdminDate } from '@/features/seller/plans-lyrium/lib/helpers';
import type { PlansMap, PlanData, AdminRequest, Vendedor, ButtonColors, DetailedBenefit, PlanFeature } from '@/features/seller/plans-lyrium/types';
...
export function useAdmin() {
  ...
  const initialize = useCallback(async () => {
    const [planesRes, requestsRes, coloresRes] = await Promise.all([
      apiGet<{ success: boolean; plans?: PlansMap }>('/planes/config.php'),
      apiGet<{ success: boolean; requests?: AdminRequest[] }>('/solicitudes/index.php'),
      apiGet<{ success: boolean; colors?: ButtonColors }>('/ui/colores.php'),
    ]);
    update({
      plansData: (planesRes.success && planesRes.plans && Object.keys(planesRes.plans).length > 0) ? planesRes.plans : defaultPlansData,
      requests:  (requestsRes.success && requestsRes.requests) ? requestsRes.requests : [],
      buttonColors: (coloresRes.success && coloresRes.colors) ? coloresRes.colors : defaultBtnColors,
      isLoaded: true,
    });
  }, [update]);
  ...
  const switchTab = useCallback(async (tab: AdminTab) => {
    ...
    if (tab === 'vendedores') {
      ...
      const vendedoresConHistorial = vendedores.map(v => ({
        ...v,
        historial: histMap[String(v.usuario_id)] ?? [],
      }));
    ...
    if (planId === 'new') {
      ...
      setState(prev => {
        ...
        return {
          ...
          editDetailedBenefits: (plan.detailedBenefits?.length ?? 0) > 0
            ? [...plan.detailedBenefits]
            : (plan.allDetails?.length ?? 0) > 0
              ? [...(plan.allDetails as import('@/features/seller/plans-lyrium/types').DetailedBenefit[])]
              : [],
        };
      });
    }
  ...
  const savePlan = useCallback(async () => {
    ...
    const newPlan: PlanData = {
      ...
      allDetails: editFeatures.filter(f => f.text?.trim()),
  ...
  const handlePagoConfirmadoAdmin = useCallback(async (datos: { planId: string; monto: number; meses: number }) => {
    ...
    if (stateRef.current.activeTab === 'vendedores') {
      ...
      update({ vendedores: vendedores.map(v => ({ ...v, historial: histMap[String(v.usuario_id)] ?? [] })) });
    }
