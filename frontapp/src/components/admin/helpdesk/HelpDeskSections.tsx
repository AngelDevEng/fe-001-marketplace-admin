                ...
                {articles.map(f => (
                    <div key={f.id} className={`${glassCardClass} p-6 flex flex-col justify-between hover:border-sky-500/50 transition-all cursor-pointer group`}>
                        <div>
                            ...
                            <h4 className="text-base font-black text-gray-800 tracking-tight mb-2 group-hover:text-sky-600 transition-colors font-industrial uppercase">{f.titulo}</h4>
                            <p className="text-xs text-gray-500 font-medium line-clamp-2 font-industrial">{f.contenido}</p>
                        </div>
                        <div className="pt-6 mt-6 border-t border-gray-50 flex justify-between items-center">
                            <div className="flex gap-3 text-[10px] font-bold text-gray-400 font-industrial">
    ...
    return (
        <div className="animate-fadeIn">
            <div className={`${glassCardClass} p-8 mb-8`}>
                <h3 className="text-xl font-black text-gray-900 mb-6 flex items-center gap-3 font-industrial uppercase">
                    ...
                    <table className="w-full text-left font-industrial">
                        <thead>
                            <tr className="bg-gray-50/50 sticky top-0 z-10 backdrop-blur-md">
                                ...
                                <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Tienda Relacionada</th>
                                <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Acción Efectuada</th>
                                <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Responsable</th>
                                <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Detalle / Justificación</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50/50">
                            {entries.map(a => (
                                <tr key={a.id} className="hover:bg-white/50 transition-all group">
                                    <td className="p-5 text-xs font-bold text-gray-500">{a.timestamp}</td>
                                    <td className="p-5 text-xs font-black text-gray-800 group-hover:text-sky-600 transition-colors">{a.tienda}</td>
                                    <td className="p-5">
                            ...
                            {entries.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="p-20 text-center text-gray-400 font-bold italic">No se encontraron registros de auditoría</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
