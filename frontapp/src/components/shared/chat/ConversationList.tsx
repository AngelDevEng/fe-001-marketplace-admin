  ...
  if (conversations.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center p-12 text-center">
        <div>
          <Icon name="Messages" className="w-12 h-12 mx-auto mb-4 text-gray-200" />
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      {conversations.map((conv) => (
        <button
          ...
          className={`w-full p-6 border-b border-gray-50 text-left transition-all relative group ${
            activeId === conv.id 
              ? accent.activeBg 
              : accent.hoverBg
          }`}
        >
          {/* Active indicator */}
          {activeId === conv.id && (
            <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-10 ${accent.indicator} rounded-r-full`} />
          )}
          ...
          <div className="flex items-start justify-between">
            ...
            {features.showUnreadCount && conv.unreadCount && conv.unreadCount > 0 && (
              <span className={`px-2 py-0.5 ${accent.badge} text-white text-[9px] font-black rounded-full flex items-center justify-center`}>
                {conv.unreadCount}
              </span>
            )}
          </div>
          
          <p className="text-[10px] text-gray-400 font-medium truncate mt-2 leading-relaxed">
            {conv.lastMessage}
          </p>
        </button>
      ))}
