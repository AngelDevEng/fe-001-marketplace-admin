    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto grid grid-cols-12 gap-0 h-full overflow-hidden">
                ...
                <aside className="col-span-12 md:col-span-4 lg:col-span-3 bg-gray-50 dark:bg-[#182420] border-r border-gray-200 dark:border-[#2A3F33] p-3 overflow-y-auto h-full">
                    <button
                        type="button"
                    ...
                    >
                        <span className="text-slate-800 dark:text-[#E8EDE9]">{activeCategory}</span>
                        <Icon name="ChevronDown" className="w-4 h-4 text-slate-500 dark:text-[#7A9A80]" />
                    </button>
                            ...
                            <li key={idx}>
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition ${activeCategory === child.label
                                        ? 'bg-white dark:bg-[#111A15] shadow-sm'
                                        : 'hover:bg-white dark:hover:bg-[#111A15] hover:shadow-sm'
                                        }`}
                                >
                                    <span className="text-slate-700 dark:text-[#E8EDE9]">{child.label}</span>
                                    <Icon
                                        name="ChevronDown"
                                        className={`w-4 h-4 text-slate-400 dark:text-[#7A9A80] ${activeCategory === child.label ? 'rotate-90' : ''
                ...
                {/* CONTENIDO DERECHO */}
                <section className="col-span-12 md:col-span-8 lg:col-span-9 p-5 overflow-y-auto h-full bg-white dark:bg-[#111A15]">
                    {/* ICONS */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        {megaData.icons.map((icon, idx) => (
                            <Link key={idx} href={icon.href} className="group text-center">
                                ...
                                <div className="mt-2 text-[12px] font-semibold text-slate-700 dark:text-[#E8EDE9] group-hover:text-sky-600 dark:group-hover:text-[#6BAF7B] transition">
                                    {icon.title}
                                </div>
                            </Link>
                        ))}
