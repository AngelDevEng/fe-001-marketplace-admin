...
export default function MegaMenu({
    ...
    return (
        <div
            className="fixed left-0 w-full bg-white shadow-2xl border-t border-sky-50 opacity-100 pointer-events-auto transition-all duration-150 z-[99999]"
        ...
        >
            <div className="max-w-7xl mx-auto grid grid-cols-12 gap-0 h-full overflow-hidden">
                ...
                <aside className="col-span-12 md:col-span-4 lg:col-span-3 bg-gray-50/70 border-r border-gray-100 p-3 overflow-y-auto h-full">
                    <button
                        type="button"
                        ...
                        {item.children.map((child, idx) => (
                            <li key={idx}>
                                ...
                                >
                                    ...
                                    <Icon
                                        name="ChevronDown"
                                        className={`w-4 h-4 text-gray-400 ${activeCategory === child.label ? 'rotate-90' : ''
                ...
                {/* CONTENIDO DERECHO */}
                <section className="col-span-12 md:col-span-8 lg:col-span-9 p-5 overflow-y-auto h-full">
                    {/* ICONS */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        {megaData.icons.map((icon, idx) => (
                            <Link key={idx} href={icon.href} className="group text-center">
                                ...
                                <div className="mt-2 text-[12px] font-semibold text-gray-700 group-hover:text-sky-600 transition">
                                    {icon.title}
                                </div>
                            </Link>
                        ))}
                    ...
                    <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 text-sm">
                        {megaData.cols.map((col, idx) => (
                            <div key={idx}>
                                <div className="text-[13px] font-extrabold tracking-wide text-gray-900 uppercase mb-2">
                                    {col.h}
                                </div>
                                <ul className="space-y-1.5">
                                    {col.items.map((it, i) => (
                                        <li key={i}>
                                            <Link
                                                href="#"
                                                className="text-[12px] text-gray-700 hover:text-sky-600 transition"
                                            >
                                                {it}
                                            </Link>
                                        </li>
