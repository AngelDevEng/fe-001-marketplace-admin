...
export default function SmartSidebar({
    ...
    if (!isMounted) return <aside className="w-20 lg:w-72 h-screen border-r border-gray-100 bg-white" />;

    return (
        <>
            <aside
                className={`
                    fixed inset-y-0 left-0 z-[60] bg-white border-r ${colors.border} flex flex-col transition-all duration-500 ease-in-out
            ...
            >
                {/* 1. PERFIL DE USUARIO */}
                <div className={`p-4 border-b ${colors.border} bg-white/95 backdrop-blur flex items-center transition-all duration-500 ${(isExpanded || isMobileOpen) ? '' : 'justify-center px-2'}`}>
                    <div className="flex items-center space-x-3 w-full">
                        ...
                        {(isExpanded || isMobileOpen) && (
                            <div className="flex flex-col min-w-0 animate-fadeIn">
                                <p className="font-black text-xs text-gray-800 leading-tight truncate uppercase tracking-tighter">
                ...
                <div className="flex items-center justify-between p-4 px-6 border-b border-gray-50 bg-gray-50/20">
                    ...
                    <button onClick={toggleSidebar} className={`hidden lg:flex p-2 hover:bg-white ${colors.accent} rounded-xl transition-all duration-300 ${!isExpanded ? 'mx-auto' : ''}`}>
                        <Icon name="ChevronLeft" className={`transition-transform duration-500 ${!isExpanded ? 'rotate-180' : ''} w-4 h-4`} />
                    </button>
                    {/* Close button for mobile inside sidebar */}
                    {onClose && (
                        <button onClick={onClose} className="lg:hidden p-2 hover:bg-white ${colors.accent} rounded-xl transition-all duration-300">
                                                ...
                                                <div className={`flex items-center h-full transition-all duration-500 ${active ? 'bg-white rounded-r-[80px] shadow-[10px_0_15px_-5px_rgba(0,0,0,0.05)]' : 'bg-transparent'}`}>
                                                    <div className={`flex items-center justify-center transition-all duration-500 ${(isExpanded || isMobileOpen) ? 'w-14' : 'w-20'}`}>
                                                        <div className={`
                                                            ...
                                                            ${active ? `${colors.bgIcon} ${colors.textActive} shadow-inner` : `bg-gray-100/50 text-gray-400 group-hover:${colors.accent} group-hover:bg-white`}
                                                        `}>
                ...
                {/* 4. FOOTER */}
                {(isExpanded || isMobileOpen) && (
                    <div className="mt-auto px-6 py-4 border-t border-gray-50 bg-gray-50/30 animate-fadeIn">
