...
export default function AgendaPage() {
    ...
    if (isLoading && events.length === 0) {
        return (
            <div className="p-20 flex flex-col items-center justify-center opacity-50 animate-pulse">
                <Icon name="Loader2" className="animate-spin text-4xl text-emerald-500 mb-4 w-10 h-10 mx-auto" />
                        ...
                        return (
                            <div
                                ...
                                className={`min-h-[140px] p-3 transition-all relative group
                                    ${cell.isOtherMonth ? 'bg-slate-50/30 opacity-20 pointer-events-none' : 'hover:bg-emerald-50/30 cursor-pointer'}
                                `}
                            >
                                <span className={`text-[15px] font-black mb-3 inline-flex items-center justify-center
                                    ${isToday ? 'text-white bg-sky-600 w-7 h-7 rounded-[10px] shadow-lg shadow-sky-100' : 'text-gray-900'}
                                `}>
                                    {cell.day}
                                </span>
