    ...
    return (
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-20 px-2 py-1 text-sm border border-emerald-300 rounded-lg focus:ring-2 focus:ring-emerald-500"
          step="0.01"
          disabled={isUpdating}
        />
        <button
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        startTransition(() => {
          setProducts(prev => {
            if (selectedProduct) {
              return prev.map(p => p.id === selectedProduct.id ? result.data! : p);
            }
