...
export default function LayoutSelector({ config, updateConfig }: LayoutSelectorProps) {
    ...
    return (
        <div className="glass-card p-0 overflow-hidden border-none rounded-[2.5rem] shadow-2xl bg-white mb-8 animate-fadeIn">
            <div className="bg-gradient-to-r from-sky-500 via-sky-500 to-sky-400 p-8 flex items-center justify-between relative overflow-hidden">
                ...
                <button
                    ...
                    className="relative z-10 flex items-center gap-3 px-6 py-3 rounded-2xl bg-white backdrop-blur-md text-black border border-white/20 font-black text-[10px] uppercase tracking-widest hover:text-sky-500 transition-all shadow-lg active:scale-95"
                >
                ...
                <div className="space-y-8">
                    <div className="space-y-6">
                        <div className="flex items-center gap-2 mb-2 ml-1">
                            ...
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Selecciona una estructura</span>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {layouts.map(layout => (
                                <label key={layout.id} className="cursor-pointer group">
                                    <input
                                        type="radio"
                                        name="layout"
                                        value={layout.id}
                                    ...
                                    />
                                    <div className="p-6 rounded-[2.5rem] border-2 border-gray-100 bg-white hover:border-sky-200 hover:shadow-lg peer-checked:border-sky-500 peer-checked:shadow-2xl peer-checked:shadow-sky-100/50 peer-checked:-translate-y-1 transition-all relative overflow-hidden">
                                        ...
                                        <div className="w-full h-40 bg-gray-50 rounded-xl p-1.5 mb-4 border border-gray-200 flex flex-col gap-0.5">
                                            {layout.preview}
                                        </div>
                                        <p className="text-[11px] font-black uppercase text-gray-500 peer-checked:text-gray-800 tracking-tight text-center transition-colors">
                                            {layout.name}
                                        </p>
                                        <p className="text-[9px] font-bold text-gray-300 uppercase mt-1 text-center transition-colors">
                                            {layout.desc}
                                        </p>
                                    </div>
                                </label>
